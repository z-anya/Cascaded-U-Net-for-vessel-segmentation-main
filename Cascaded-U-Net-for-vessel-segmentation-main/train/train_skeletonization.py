...
parser.add_argument('--dir_data', metavar='dir_data', type=str, nargs="?", default='/opt/data/private/DATA', help='Data\'s directory')
...
dir_inputs = os.path.join(dir_data, 'WORD/WORD-V0.1.0/imagesTr')
dir_GT = os.path.join(dir_data, 'cnterline/WORD/colon_skeleton_smooth')

# Segmentation as input to perform skeletonization
dir_inputs_seg = os.path.join(dir_data, 'WORD/word2colon/labelTr_colon')
...
for (root, directory, file) in os.walk(dir_inputs):
    for f in file:
        if f.split('-')[0] in patient_train:
            ...
            data_train.append(dict(zip(['image', 'segmentation', 'GT'],  [os.path.join(dir_inputs, f), os.path.join(dir_GT, gt_filename)])))
            
for (root, directory, file) in os.walk(dir_inputs):
    for f in file:
        if f.split('-')[0] in patient_test:
            gt_filename = get_gt_filename(f)
            data_test.append(dict(zip(['image', 'segmentation', 'GT', 'filename'], [os.path.join(dir_inputs, f), os.path.join(dir_GT, gt_filename), f])))
