...
from monai.transforms import Transform, AddChanneld, LoadImaged, EnsureTyped, ScaleIntensityRanged, Lambda, NormalizeIntensityd, RandSpatialCropd, adaptor, SqueezeDimd, Compose, EnsureChannelFirstd
...
parser.add_argument('--batch_size', metavar='batch_size', type=int, nargs="?", default=1, help='Batch size for training phase')
parser.add_argument('--learning_rate', metavar='learning_rate', type=float, nargs="?", default=0.001, help='Learning rate for training phase')
...
parser.add_argument('--dir_data', metavar='dir_data', type=str, nargs="?", default='/opt/data/private/DATA', help='Data directory')
...
dir_inputs = str(Path(args.dir_data) / 'WORD/WORD-V0.1.0/imagesTr')
dir_GT = str(Path(args.dir_data) / 'WORD/word2colon/labelTr_colon')

# 修改为实际文件名格式
def get_gt_filename(image_filename):
    base_name = os.path.basename(image_filename)
    # 假设图像文件名是类似 word_0003.nii.gz 的格式
    patient_id = base_name.split('.')[0]
    return f"label_10_{patient_id}.nii.gz"
...
for (root, directory, file) in os.walk(dir_inputs):
    for f in file:
        if f.split('-')[0] in patient_train:
            gt_filename = get_gt_filename(f)
            data_train.append(dict(zip(['image', 'GT'], [os.path.join(dir_inputs, f), os.path.join(dir_GT, gt_filename)])))

for (root, directory, file) in os.walk(dir_inputs):
    for f in file:
        if f.split('-')[0] in patient_test:
            gt_filename = get_gt_filename(f)
            data_test.append(dict(zip(['image', 'GT', 'filename'], [os.path.join(dir_inputs, f), os.path.join(dir_GT, gt_filename), f])))
...
class NormalizeCTIntensityd(Transform):
    def __init__(self, keys):
        self.keys = keys

    def __call__(self, data):
        for key in self.keys:
            data[key] = self.normalize_ct(data[key])
        return data

    @staticmethod
    def normalize_ct(image):
        # Clip HU values to the range [-200, 200] and normalize to [0, 1]
        image = np.clip(image, -200, 200)
        image = (image + 200) / 400.0
        return image
...
transform_io = [
    ...
    NormalizeCTIntensityd(keys=['image']),  # 直接对图像应用归一化
    RandSpatialCropd(keys, roi_size=patch_size, random_size=False)
]
# Data augmentation
...
print(f"Epoch {1}\n-------------------------------")
epoch_save = 1
logs = train_loop(train_data, val_data, model=model, loss_param=loss_param, input_='CT', optimizer=optimizer, device=device, epoch=1, max_epoch=epochs, alpha_=alpha)
...
for t in range(1, epochs):
    ...
    logs = train_loop(train_data, val_data, model=model, loss_param=loss_param, input_='CT', optimizer=optimizer, device=device, epoch=t + 1, max_epoch=epochs, alpha_=alpha)
